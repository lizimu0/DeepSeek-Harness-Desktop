window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
			...
			.dbc-heat-months{display:grid;grid-auto-flow:column;grid-auto-columns:20px;gap:5px;font-size:12px;opacity:.6;height:16px;margin:0 0 4px 25px}
			.dbc-heat-months span{white-space:nowrap;justify-self:start}
			.dbc-heat-body{display:flex;gap:5px;align-items:flex-start}
			.dbc-heat-wk{display:grid;grid-template-rows:repeat(7,20px);gap:5px;font-size:10px;opacity:.45;width:20px}
			.dbc-heat-wk span{display:flex;align-items:center;line-height:1}
			/* Codex/GitHub 式固定 20px 小格子：一列一周、一行一星期，左对齐不拉伸 */
			.dbc-heat-grid{display:grid;grid-auto-flow:column;grid-template-rows:repeat(7,20px);grid-auto-columns:20px;gap:5px}
			.dbc-heat-cell{width:20px;height:20px;border-radius:5px;border:1px solid transparent;transition:transform .12s,border-color .12s;cursor:default}
			...
			.dbc-heat-legend{display:flex;gap:4px;align-items:center;justify-content:flex-end;font-size:11px;opacity:.7;margin-top:9px}
			.dbc-heat-legend i{width:14px;height:14px;border-radius:4px;display:inline-block}
